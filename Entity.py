...
class Entity(object):
    ...
    def __init__(self, x: float = 0, y: float = 0, speed: float = 0, angle: float = math.pi/2, file: str = None):
        ...
        self._speed = speed
        self._angle = angle 
        self._vector = pmath.Vector2(speed * math.cos(angle), speed * math.sin(angle))
        self._rotation = 0
        self._tick = 0
        self._display = True
        self._animate = False
        ...
        if file is not None:
            self._sprite.append(self.load(file))
    ...
    # def _setdxdy(self) -> None:
    #     self._dx = self.speed * math.cos(self._angle)
    #     self._dy = self.speed * math.sin(self._angle)
    ...
    def update(self) -> None:
        self._tick += 1
        if self._animate and self._tick % self._delayTicks == 0:
            self.index += 1
            if self.index == 0:
                if not self.loop:
                    self._animate = False
